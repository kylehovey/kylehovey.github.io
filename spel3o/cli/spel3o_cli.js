...
TerminalShell.commands['shutdown'] = TerminalShell.commands['poweroff'] = function(terminal) {
	if (this.sudo) {
		terminal.print('Broadcast message from guest@haversine');
		terminal.print();
...
Filesystem = {
	'welcome.txt': {type:'file', read:function(terminal) {
		terminal.print($('<h4>').text('Welcome to the spel3o console.'));
		terminal.print('Code originally written by Chromakode for xkcd under the GNU public license.');
...
Filesystem['blog'] = Filesystem['8bitproject'] = linkFile('http://8bitspaghetti.com');
Filesystem['music'] = Filesystem['band'] = linkFile('http://thestereosix.com');
Filesystem['facebook'] = linkFile('http://facebook.com/192.168.kyle.hovey');
Filesystem['minecraft'] = linkFile('http://mc.spel3o.com');
...
TerminalShell.commands['cheat'] = function(terminal) {
	terminal.print($('<a>').text('*******YOU WON THE GAME!!!*******').attr('href', 'http://no.pe/'));
}; 
...
TerminalShell.commands['irc'] = function(terminal, nick) {
	if (nick) {
		...
		var url = "http://widget.mibbit.com/?server=mc.spel3o.com&channel=%23minecraft";
		if (nick) {
